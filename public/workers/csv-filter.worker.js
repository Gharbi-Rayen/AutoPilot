...
// src/workers/csv-filter.worker.ts
function applyFilter(row, condition) {
  const raw = row[condition.field];
  const cell = raw === null || raw === void 0 ? "" : String(raw);
...
async function readDatasetFromOPFS(executionId, datasetId, chunkCount) {
...
self.onmessage = async (event) => {
  ...
  const { inputRef, conditions, logic = "AND", executionId, variableName, chunkSize = 1e4 } = input;
  ...
  try {
    ...
    const filtered = rows.filter(
      (row) => logic === "AND" ? conditions.every((c) => applyFilter(row, c)) : conditions.some((c) => applyFilter(row, c))
    );
    ...
    const datasetRef = { kind: "dataset", datasetId, executionId, variableName, rowCount: filtered.length, chunkCount: chunks.length, byteSize: totalBytes, schema: inputRef.schema };
