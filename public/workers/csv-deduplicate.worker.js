...
self.onmessage = async (event) => {
  ...
  const { inputRef, keyFields, executionId, variableName, chunkSize = 1e4 } = input;
  ...
  try {
    ...
    for (const row of rows) {
      const key = keyFields.length > 0 ? keyFields.map((f) => String(row[f] ?? "")).join("|") : JSON.stringify(row);
      if (!seen.has(key)) {
        seen.add(key);
        deduped.push(row);
      }
    ...
    const datasetRef = { kind: "dataset", datasetId, executionId, variableName, rowCount: deduped.length, chunkCount: chunks.length, byteSize: totalBytes, schema: inputRef.schema };
