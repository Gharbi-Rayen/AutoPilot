...
// src/workers/csv-consecutive-sequence.worker.ts
self.onmessage = async (event) => {
  const { jobId, input } = event.data;
  const { inputRef, sequenceField, groupByField, executionId, variableName, chunkSize = 1e4 } = input;
  ...
  try {
    ...
    for (const [groupKey, groupRows] of groups) {
      const nums = groupRows.map((r) => ({ row: r, val: Number(r[sequenceField]) })).filter((x) => !Number.isNaN(x.val)).sort((a, b) => a.val - b.val);
