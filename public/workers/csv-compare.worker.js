...
self.onmessage = async (event) => {
  ...
  const { baseRef, compareRef, keyFields, executionId, variableName, chunkSize = 1e4 } = input;
  ...
  try {
    ...
    const compare2 = await readFromOPFS(compareRef.executionId, compareRef.datasetId, compareRef.chunkCount);
    post({ kind: "progress", jobId, progress: 50, message: "Comparing..." });
    const rowKey = (r) => keyFields.map((f) => String(r[f] ?? "")).join("|");
    const baseMap = new Map(base.map((r) => [rowKey(r), r]));
    const compareMap = new Map(compare2.map((r) => [rowKey(r), r]));
    const added = [];
    const removed = [];
    const changed = [];
    for (const [key, row] of compareMap) {
      if (!baseMap.has(key)) {
        added.push({ ...row, _diff: "added" });
      } else if (JSON.stringify(row) !== JSON.stringify(baseMap.get(key))) {
        changed.push({ ...row, _diff: "changed" });
      }
    }
    ...
    const datasetRef = { kind: "dataset", datasetId, executionId, variableName, rowCount: diffRows.length, chunkCount: chunks.length, byteSize: totalBytes };
