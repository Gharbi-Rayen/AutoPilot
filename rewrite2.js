...
code = code.replace(/const \[isMetadataCollapsed, setIsMetadataCollapsed\] = useState\(true\);\n/, 'const [isMetadataCollapsed, setIsMetadataCollapsed] = useState(true);\n  const [isOutputExpanded, setIsOutputExpanded] = useState(false);\n');
...
code = code.replace(/const requiresRawOutputForSelection =\n?\s*traceSelection\?\.kind === "workflow-node";/, '');
...
code = code.replace(/\s*setIsRawOutputRequested\(false\);\n/, '\n');
code = code.replace(/\s*if \(!isRawOutputRequested\) \{[^}]*return;\n\s*\}/, '');
code = code.replace(/\[executionRawOutputQuery\.data, isRawOutputRequested, setExecutionResult\]/, '[executionRawOutputQuery.data, setExecutionResult]');
code = code.replace(/const handleLoadRawOutput = \(\) => \{[^}]*if \(!activeExecutionId\) \{[^}]*return;\n\s*\}[^}]*setIsRawOutputRequested\(true\);\n\s*\};\n/, '');

const targetStart = '<div>\n                        {requiresRawOutputForSelection &&';
const idxStart = code.lastIndexOf(targetStart);
const strEnd = 'No output for this selection.\n                        </div>\n                      )}';
const idxEnd = code.indexOf(strEnd) + strEnd.length;

if (idxStart !== -1 && idxEnd !== -1) {
  const replacement = `executionRawOutputQuery.isFetching ? (
