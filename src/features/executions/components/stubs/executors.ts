...
export const pdfFillFormExecutor: NodeExecutor =
  pendingExecutor("pdf_fill_form");
...
export const pdfExtractTablesExecutor: NodeExecutor =
  pendingExecutor("pdf_extract_tables");
...
export const csvAggregateExecutor: NodeExecutor<CsvAggregateData> = async ({
  ...
  withFileNodeStatus(nodeId, publish, async () => {
    ...
    const aggregated = await step.run("csv-aggregate", async () => {
      ...
      const records = Array.from(groups.entries()).map(
        ([groupKey, groupRows]) => {
...
export const jsonTransformExecutor: NodeExecutor =
  pendingExecutor("json_transform");
