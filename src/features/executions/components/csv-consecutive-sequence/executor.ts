...
export const executor: NodeExecutor = async (_nodeId, nodeData, context, executionId, onProgress) => {
  const { inputVariable, sourceVariable, variableName = "sequenceData", ...rest } = nodeData as Record<string, unknown>;
  ...
  if (!isDatasetRef(inputRef)) throw new Error("csv-consecutive-sequence: no dataset in context.");
  const result = await dispatchWorkerJob<unknown, { datasetRef: DatasetRef; manifest: unknown }>("csv-consecutive-sequence", { inputRef, ...rest, executionId, variableName }, onProgress);
  return {
