...
export const executor: NodeExecutor = async (_nodeId, nodeData, context, executionId, onProgress) => {
  const { inputVariable, sourceVariable, variableName = "aggregatedData", ...rest } = nodeData as Record<string, unknown>;
  ...
  if (!isDatasetRef(inputRef)) throw new Error("csv-aggregate: no dataset in context.");
  const result = await dispatchWorkerJob<unknown, { datasetRef: DatasetRef; manifest: unknown }>("csv-aggregate", { inputRef, ...rest, executionId, variableName }, onProgress);
  return {
