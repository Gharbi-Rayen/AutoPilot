...
export const executor: NodeExecutor = async (_nodeId, nodeData, context, executionId, onProgress) => {
  const {
    ...
    sortField, direction, compareAs, nulls,
    ...rest
  ...
  const sortColumns = sortField
    ? [{ field: sortField as string, direction: (direction as "asc" | "desc") ?? "asc" }]
    : [];
  const result = await dispatchWorkerJob<unknown, { datasetRef: DatasetRef; manifest: unknown }>("csv-sort", { inputRef, ...rest, sortColumns, compareAs, nulls, executionId, variableName }, onProgress);
  return {
