...
export const executor: NodeExecutor = async (_nodeId, nodeData, context, executionId, onProgress) => {
  const { inputVariable, sourceVariable, variableName = "filteredData", ...rest } = nodeData as Record<string, unknown>;
  ...
  const result = await dispatchWorkerJob<unknown, { datasetRef: DatasetRef; manifest: unknown }>("csv-filter", { inputRef, ...rest, executionId, variableName }, onProgress);
  return {
